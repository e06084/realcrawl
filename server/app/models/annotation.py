...
class AnnotationUpdate(BaseModel):
    ...
    annotations: Optional[str] = Field(None, description="标注数据")
    notes: Optional[str] = Field(None, description="备注")
    status: Optional[AnnotationStatus] = Field(None, description="状态")
    # 注意：user_id不允许更新，由权限控制确保用户只能更新自己的标注
...
class AnnotationSearchRequest(BaseModel):
    ...
    url: Optional[str] = Field(None, description="URL过滤")
    ...
    # 注意：user_id过滤由middleware自动添加，确保用户只能搜索自己的标注
